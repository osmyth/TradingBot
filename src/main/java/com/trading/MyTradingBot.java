    ...
    public FixedPointNumber getValuationBidPrice(long startTime, long endTime, boolean lastHigh) {
        ...
        for (Long key : data.keySet()) {
            ...
            if (key < endTime && key >= startTime) {
                ...
                // if we are looking for the last highest bid for the period, then check if the bid is greater than the current value
                // else if we are looking for the last lowest bid for the period, then check if the bid is lower than the current value
